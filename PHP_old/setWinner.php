...
$setLosses = "UPDATE `accounts` JOIN `playersingame` ON `accounts`.`username`=`playersingame`.`name` SET `losses`=`losses`+1 WHERE username != (SELECT `name` FROM `playersingame` WHERE `score` = (SELECT MAX(`score`) FROM `playersingame`));";
